...
class FeignCallObfuscatingLoggerSpec extends MicroserviceMvcWiremockSpec {
    ...
    def 'JSON: Should create log REQ/RES with all HTTP elements for GET json message'() {
        given:
            final Appender mockAppender = insertAppender(Mock(Appender))
            stubGet('/logTestJson', 200, readResource(JSON_REQ_RESOURCE_NAME),
    ...
    def 'JSON: Should not create log REQ/RES for GET call due to skip configuration'() {
        given:
            final Appender mockAppender = insertAppender(Mock(Appender))
            stubGet('/logTestJsonSkip', 200, readResource(JSON_REQ_RESOURCE_NAME),
                    [(CONTENT_TYPE as String): MediaType.APPLICATION_JSON.toString()])
        when:
        ...
        then:
            interaction {
                ...
                checkInteraction(mockAppender,'.*Cannot obfuscate response, matching request data lost.*', 0)
            }
        cleanup:
            removeAppender(mockAppender)
    }
    ...
    def 'JSON: Should create log REQ/RES for POST call due to skip configuration only for GET'() {
        ...
        then:
            interaction {
                checkInteraction(mockAppender,
                        '.*REQ CLIENT->method.*POST.*url.*/logTestJson.*headers.*application/json.*body.*affiliates.*', 1)
                and:
                checkInteraction(mockAppender, '.*RES CLIENT<-headers.*status.*201.*', 1)
                checkInteraction(mockAppender,'.*Cannot obfuscate response, matching request data lost.*', 0)
            }
    ...
    def 'JSON: Should skip logging response if request data is gone from cache'() {
        ...
        then:
            interaction {
                ...
                checkInteraction(mockAppender,'.*Cannot obfuscate response, matching request data lost.*', 1)
            }
        cleanup:
            removeAppender(mockAppender)
    }
