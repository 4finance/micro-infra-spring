...
class ProjectMetadataParserSpec extends Specification {
    ...
    public static final ProjectMetadata PL_DESCRIPTOR_PROJECT = new ProjectMetadata('descriptor', 'com/ofg/foo', PL_CONTEXT)
    public static final ProjectMetadata PL_BROKER_PROJECT = new ProjectMetadata('brokers', 'com/ofg/bar', PL_CONTEXT)
    public static final ProjectMetadata PL_ANOTHER_DESCRIPTOR_PROJECT = new ProjectMetadata('anotherDescriptor', 'com/ofg/foo/bar', PL_CONTEXT)
    ...
    def 'should parse project metadata'() {
        given:
            Collection<ProjectMetadata> expectedProjects = [PL_HELLO_PROJECT, PL_BYE_PROJECT, LV_PING_PROJECT]
        when:
            Collection<ProjectMetadata> projects = ProjectMetadataResolver.resolveFromMetadata(METADATA)
        then:
    ...
    def 'should parse project metadata files that resides under default folder'() {
        given:
            ...
            Collection<ProjectMetadata> expectedProjects = [PL_DESCRIPTOR_PROJECT, PL_BROKER_PROJECT, PL_ANOTHER_DESCRIPTOR_PROJECT]
        when:
            Collection<ProjectMetadata> projects = ProjectMetadataResolver.resolveAllProjectsFromRepository(repository, PL_CONTEXT)
        then:
            projects.size() == expectedProjects.size()
            projects.containsAll(expectedProjects)
    }
